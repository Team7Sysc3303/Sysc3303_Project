...
public class Server implements Runnable {
   ...
   private DatagramPacket sendPacket, receivePacket, oldPacket = null;
 ...
 public boolean comparePacket(DatagramPacket a, DatagramPacket b){
	 ...
	 //  String filename = "", mode = "";
	   ...
	   for(;;) { // loop forever
	         ...
	         // Block until a datagram packet is received from receiveSocket.
	         	...
	         	// this to deal with duplicated requests.
		         try {
...
if(oldPacket == null || receivePacket.equals(oldPacket)){
	      // If it's a read, send back DATA (03) block 1
	         ...
	         if (data[0]!=0) req = Request.ERROR; // bad
		         ...
		         if(req!=Request.ERROR) { // check for mode
		            ...
		            if (s==i+1) req=Request.ERROR; // mode is 0 bytes long
		            mode = new String(data,i,s-i-1);
		            responseThread = new Thread (new ResponseHandler(req, receivePacket, filename, mode), "ThreadSend" + counter);//new
	        ...
	        // Thread responseThread = new Thread (new ResponseHandler(req, receivePacket, filename, mode), "ThreadSend" + counter);
	         counter ++;
} // end of checking if duplicated.
else{
	System.out.println("Server: Request is duplicated and ignored.");
}
	   }
   }
   public void run(){
